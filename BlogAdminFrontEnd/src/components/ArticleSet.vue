    ...
    <div>
        ...
        <el-dialog title="新增文章" :visible.sync="addDialogVisible" width="90%" @close="handleAddClose" :close-on-click-modal="false">
            <el-form :model="addArticleForm" :rules="addArticleRules" ref="addArticleRef" label-width="100px">
                ...
                <el-form-item label="标签" prop="tag">
                    <el-select v-model="addArticleForm.tag" multiple collapse-tags placeholder="请选择标签" style="width: 450px" clearable filterable @change="createMainList" @clear="clearMainList">
                ...
                <el-form-item label="主标签" prop="mainTag">
                    <el-select v-model="addArticleForm.mainTag" collapse-tags placeholder="请选择主标签" style="width: 450px" clearable filterable>
        ...
        <el-dialog title="编辑文章" :visible.sync="editDialogVisible" width="90%" :close-on-click-modal="false">
            ...
            <el-form :model="editArticleForm" :rules="editArticleRules" ref="editArticleRef" label-width="100px">
                ...
                <el-form-item label="标签" prop="tag">
                    <el-select v-model="editArticleForm.tag" multiple collapse-tags placeholder="请选择标签" style="width: 450px" clearable @change="createMainList" @clear="clearMainList">
                        <el-option v-for="item in TagOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="主标签" prop="mainTag">
                    <el-select v-model="editArticleForm.mainTag" collapse-tags placeholder="请选择主标签" style="width: 450px" clearable>
                        <el-option v-for="item in MainList" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="日期" prop="publish_time">
...
export default {
    data(){
        return {
            ...
            addArticleRules: {
                ...
                mainTag: [{ required: true, message: '请选择主标签', trigger: 'blur' },],
    ...
    methods: {
        ...
        queryCategoryTag(){
            this.$http.get('admin/article/map').then(response => {
                if (response.status === 200){
                    const res = response.data;
                    if (res.status_code === 1000){
        ...
        createMainList(val){
            ...
            for (let i=0; i < val.length; i++){
                for (let j = 0; j < this.TagOptions.length; j++){
                    if (val[i] === this.TagOptions[j].value){
                        MainListMap[val[i].toString()] = this.TagOptions[j]
                    }
                }
            }
            ...
            for (let item in MainListMap){
                this.MainList.push(MainListMap[item])
            }
           
        },

        clearMainList(){
            this.addArticleForm.mainTag = ""
            this.editArticleForm.mainTag = ""
        },
        ...
        handleDeleteClick(row){
            this.$confirm('此操作将永久删除该文章, 是否继续?', '提示', {
                ...
                this.$http.get('admin/article/delete', {params: {id: row.id}}).then(response => {
                    if (response.status === 200){
                        const res = response.data;
                        if (res.status_code === 1000){
                            this.$message.success(res.message);
        ...
        handleEditClick(row){
            ...
            this.editArticleForm.aid = row.id
            ...
            this.editArticleForm.mainTag = row.main_tag
            ...
            for (let i=0; i < row.tag.length; i++){
                for (let j = 0; j < this.TagOptions.length; j++){
                    if ( row.tag[i] === this.TagOptions[j].value){
                        MainListMap[row.tag[i].toString()] = this.TagOptions[j]
                    }
                }
            }
            this.MainList = []
